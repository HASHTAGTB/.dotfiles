...
#
# Usage:
#   theme_ctl set --mode anime --type scheme-vibrant
#   theme_ctl set --index 1 --base16 wal
#   theme_ctl set --no-wall --mode safe
#   theme_ctl next
#   theme_ctl prev
#   theme_ctl random
#   theme_ctl refresh
...
readonly TRACK_SAFE="${STATE_DIR}/safe_wal"
readonly TRACK_ANIME="${STATE_DIR}/anime_wal"

readonly BASE_PICTURES="${HOME}/Pictures/wallpapers"
readonly STORED_SAFE_DIR="${BASE_PICTURES}/safe"
readonly STORED_ANIME_DIR="${BASE_PICTURES}/anime"
readonly WALLPAPER_ROOT="${BASE_PICTURES}/all"
...
move_directories() {
    ...
    case "$target_mode" in
        anime)
            source_dir="$STORED_ANIME_DIR"
            stash_dir="$STORED_SAFE_DIR"
            ;;
        safe)
            source_dir="$STORED_SAFE_DIR"
            stash_dir="$STORED_ANIME_DIR"
            ;;
        *)
...
regenerate_current() {
    ...
    if [[ ! -f "$resolved_wallpaper" && "$current_wallpaper" == "$ACTIVE_THEME_DIR/"* ]]; then
        ...
        if [[ "$THEME_MODE" == "anime" ]]; then
            primary_store="$STORED_SAFE_DIR"
            secondary_store="$STORED_ANIME_DIR"
        else
            primary_store="$STORED_ANIME_DIR"
            secondary_store="$STORED_SAFE_DIR"
        fi
...
Examples:
  theme_ctl set --mode anime --index 1 --base16 wal
  theme_ctl set --no-wall --mode safe
  theme_ctl next
  theme_ctl prev
  theme_ctl random
    ...
    while (( $# > 0 )); do
        case "$1" in
            --mode)
                ...
                [[ "$2" == "safe" || "$2" == "anime" ]] || die "--mode must be 'safe' or 'anime'"
                desired_mode="$2"
